...
class EstatePropertyThirteen(models.Model):
    _name = "estate.property.thirteen"
    _description = "estate property model thirteen"
    ...
    property_type_id = fields.Many2one(
        "estate.property.type.thirteen", string="Property Type", required=True
    )
    sales_person = fields.Many2one(
        "res.users", string="Sales person", default=lambda self: self.env.user
    )
    buyer = fields.Many2one("res.partner", string="Buyer", copy=False)
    tag_ids = fields.Many2many("estate.property.tag.thirteen", string="tags")
    offer_ids = fields.One2many(
        "estate.property.offer.thirteen", "property_id", string=""
