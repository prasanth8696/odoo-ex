...
class EstatePropertyEleven(models.Model):
    _name = "estate.property.eleven"
    _description = "estate property model eleven"
    ...
    property_type_id = fields.Many2one(
        "estate.property.type.eleven", string="Property Type", required=True
    )
    sales_person = fields.Many2one(
        "res.users", string="Sales person", default=lambda self: self.env.user
    )
    buyer = fields.Many2one("res.partner", string="Buyer", copy=False)
    tag_ids = fields.Many2many("estate.property.tag.eleven", string="tags")
    offer_ids = fields.One2many(
        "estate.property.offer.eleven", "property_id", string="Offer"
    )
